...
class Level5 extends Component {
  ...
  reduceLives() {
    ...
    if(this.numHearts==0){
      ...
      fetch(`/add-log-entry?level=5&algorithm=merge&completed=0&mistakes=3&timeSpent=${this.timerElement.current.state.time}`).then((result) => {console.log(result)});
    }
    stepCounter = 0;
  ...
  handleLevelComplete = () => {
    ...
    fetch(`/add-log-entry?level=5&algorithm=merge&completed=1&mistakes=${3 - this.numHearts}&timeSpent=${this.timerElement.current.state.time}`).then((result) => {console.log(result)});
    this.setState({ showModalPopup: true });
